...
var spreadsheet = SpreadsheetApp.openById('1erPqcKImplsuWLgCxempUt433eJt2kWuigej_YQ4KGY');
// Test version
...
/**
 * Generates a month suffix for sheet names based on current date
...
/**
 * Generates sheet name with month suffix
 */
function getMonthlySheetName(baseSheetName) {
  var monthSuffix = getMonthSuffix();
  return baseSheetName + '_' + monthSuffix;
}

/**
 * Aggregates daily data into monthly totals
 * Assumes column B contains dates and other columns contain numeric values to sum
 */
function aggregateToMonthly(data) {
  ...
  // Find the date column (assume it's column B, index 1)
  ...
  dataRows.forEach(function(row) {
    var dateValue = row[dateColumnIndex];
    ...
    if (!monthlyGroups[yearMonth]) {
      monthlyGroups[yearMonth] = {
        date: yearMonth,
        values: new Array(row.length - 2).fill(0), // Skip date column, initialize other columns to 0
        count: 0
      };
    }
    
    // Sum numeric columns (skip the date column)
    for (var i = 2; i < row.length; i++) {
      var value = parseFloat(row[i]) || 0;
      monthlyGroups[yearMonth].values[i - 2] += value;
    }
    monthlyGroups[yearMonth].count++;
  });
  
  // Convert grouped data back to array format
  var aggregatedData = [headers]; // Keep original headers
  
  // Sort months chronologically
  var sortedMonths = Object.keys(monthlyGroups).sort();
  
  sortedMonths.forEach(function(yearMonth) {
    var group = monthlyGroups[yearMonth];
    var row = [headers[0] || 'Month', yearMonth]; // First column might be a label, second is the date
  ...
  try {
    ...
    for (var i = 0; i<reports.length; i++) {
      try {
        ...
        var aggregatedData = aggregateToMonthly(tableData);
        
        // Generate monthly sheet name instead of using the base name
        var monthlySheetName = getMonthlySheetName(reports[i].spreadsheetTab);
        tableObj[monthlySheetName] = aggregatedData;
      }
      catch (e) { 
        if (e == 'Query failed!') {
...
/**
 * Creates or gets a sheet for monthly data, positioning it at the end of the spreadsheet
 */
function createOrGetMonthlySheet(sheetName) {
  var sheet = spreadsheet.getSheetByName(sheetName);
  
  if (!sheet) {
    // Create the sheet and move it to the end
    sheet = spreadsheet.insertSheet(sheetName);
    // Move the new sheet to the end (last position)
    ...
    Logger.log('Using existing monthly sheet: ' + sheetName);
  }
...
function sendToSpreadsheet(tableObj) {
  ...
  try {
    ...
    Object.keys(tableObj).forEach(function(tableKey) {
      var sheetName = tableKey; // From the main fn, each data object should have as its key a monthly spreadsheet tab name
      
      // Create or get the monthly sheet (will be positioned at the end)
      var sheet = createOrGetMonthlySheet(sheetName);
      
      var data = tableObj[tableKey];
      
      // Check if sheet already has data for this month (to avoid duplicates)
      var existingData = sheet.getDataRange().getValues();
      if (existingData.length > 1 && existingData[0].length > 0) {
        Logger.log('Sheet ' + sheetName + ' already contains data. Overwriting...');
      }
      ...
      try { 
        ...
        Logger.log('Successfully populated sheet: ' + sheetName + ' with ' + data.length + ' rows');
      }
      catch (e) {
        Logger.log('Error setting values in sheet ' + sheetName + ': ' + e);
      }
    });
  }
  catch (e) {
    //ui.alert("Loading spreadsheet failed!");
    Logger.log('Error in sendToSpreadsheet: ' + e);
  }
}

/**
 * Optional: Function to clean up old monthly sheets (keep only last N months)
 * Call this function if you want to automatically remove older monthly data
 */
function cleanupOldMonthlySheets(monthsToKeep) {
  monthsToKeep = monthsToKeep || 12; // Default to keeping 12 months of data
  ...
  allSheets.forEach(function(sheet) {
    ...
    // Skip config sheet and sheets that don't follow the monthly naming pattern
    if (sheetName === 'config' || !sheetName.includes('_20')) {
      return;
    }
    
    // Extract date from sheet name (assumes format: SheetName_YYYY-MM)
    var dateParts = sheetName.match(/_(\d{4})-(\d{2})$/);
    if (!dateParts) {
      return; // Skip sheets that don't match the pattern
    }
    
    var year = parseInt(dateParts[1]);
    var month = parseInt(dateParts[2]) - 1; // JavaScript months are 0-indexed
    var sheetDate = new Date(year, month, 1);
    
    // Calculate months difference
    var monthsDiff = (currentDate.getFullYear() - sheetDate.getFullYear()) * 12 + 
                     (currentDate.getMonth() - sheetDate.getMonth());
  ...
  sheetsToDelete.forEach(function(sheet) {
    Logger.log('Deleting old monthly sheet: ' + sheet.getName());
    spreadsheet.deleteSheet(sheet);
  });
  
  if (sheetsToDelete.length > 0) {
    Logger.log('Cleaned up ' + sheetsToDelete.length + ' old monthly sheets');
  }
}
